...
export default function render({ login, profile, total }, options = {}) {
  ...
  // ---------- typing timeline ----------
  const CW = 8.4, TY = 150;
  const { lines: typed, end: DURATION } = scheduleLines(lines);
  ...
  out.push(typewriter(typed, { x: (l) => W / 2 - (cellWidth(l.text) * CW) / 2, y: TY, cw: CW, anim, prefix: "t", cls: "line", cursor: { w: 9, h: 19, color: "#7ee8fa" } }));
