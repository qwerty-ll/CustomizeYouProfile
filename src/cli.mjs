...
//
//   GITHUB_TOKEN=... node src/cli.mjs --login octocat --effects dino-run,fireworks \
//     --out profile-effects --readme README.md --language en

import { mkdir, readFile, writeFile } from "node:fs/promises";
import { dirname, join, relative } from "node:path";
import { parseArgs } from "node:util";
import { buildContext, fetchCalendar } from "./lib.mjs";
import { updateReadme } from "./readme.mjs";

export const EFFECTS = {
  "dino-run": { title: "Dino run", alt: "A dino running through the year, jumping over commit-cacti" },
  "fireworks": { title: "Fireworks", alt: "Fireworks: every active day launches a rocket" },
  "black-hole": { title: "Black hole", alt: "A black hole swallows the contribution graph and a big bang rebuilds it" },
  "solar-system": { title: "Solar system", alt: "Solar system: months are planets, active days are moons" },
  "oscilloscope": { title: "Oscilloscope", alt: "CRT oscilloscope tracing daily commits" },
  "terminal": { title: "Terminal", alt: "Terminal typing git log --stats" },
  "notebook": { title: "Notebook", alt: "Squared school notebook shaded in pencil" },
  "space-shooter": { title: "Space shooter", alt: "A spaceship shooting down contributions, biggest first" },
};
...
for (const id of requested) {
  ...
  const files = render(ctx, { language });
